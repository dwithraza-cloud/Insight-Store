...
export async function POST(request:Request){
  ...
  if(!proof.startsWith("/api/media/payment-proofs/"))return Response.json({error:"Payment screenshot is required."},{status:400});
  ...
  if(transaction){const existing=await env.DB.prepare("SELECT id FROM store_orders WHERE transaction_reference=?").bind(transaction).first();if(existing)return Response.json({error:"This transaction ID has already been submitted."},{status:409});}
...
export async function PATCH(request:Request){await schema();const body=await request.json() as any;const order=await env.DB.prepare("SELECT * FROM store_orders WHERE order_number=? AND access_token=?").bind(clean(body.order_number,40),clean(body.access_token,80)).first<any>();if(!order)return Response.json({error:"Order not found."},{status:404});if(order.payment_status!=="Payment Rejected")return Response.json({error:"New proof can only be submitted for a rejected payment."},{status:409});const ref=clean(body.transaction_reference,100),proof=clean(body.proof_url,700);if(!proof.startsWith("/api/media/payment-proofs/"))return Response.json({error:"New payment screenshot is required."},{status:400});await env.DB.prepare("UPDATE store_orders SET transaction_reference=?,proof_url=?,payment_status='Verification Pending',order_status='Payment Pending',admin_note=NULL,verified_by=NULL,verified_at=NULL,updated_at=? WHERE id=?").bind(ref,proof,Date.now(),order.id).run();return Response.json({ok:true})}
